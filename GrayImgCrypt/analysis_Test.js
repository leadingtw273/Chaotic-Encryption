...
class CCA {
  ...
  compute(dataA, dataB) {
    return (
      this.cov(dataA, dataB) /
      (Math.sqrt(this.D(dataA)) * Math.sqrt(this.D(dataB)))
    ).toFixed(this.round);
  }
  ouputCSV(Data) {
    // 輸出csv檔
    ...
    let writeStream = fs.createWriteStream(
      this.reportPath + `/${this.typeFile}_CCA.csv`
    );
    writeStream.write(
      'INDEX,HOR_dataA,HOR_dataB,VER_dataA,VER_dataB,DIA_dataA,DIA_dataB\n'
    );
...
class IE {
  ...
  H(arrayData) {
    ...
    // 輸出csv檔
    let writeStream = fs.createWriteStream(
      this.reportPath + `/${this.typeFile}.csv`
    );
